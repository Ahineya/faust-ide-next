...
import primitives from "./primitives.lib";

import {docSections, faustDocURL} from "../../../../Downloads/faustide-master/src/documentation";
...
export const theme: editor.IStandaloneThemeData = {
  ...
  rules: [
    ...
    {token: "faustKeywords", foreground: "cc7832"},
    {token: "faustLib", foreground: "9876aa"},
    {token: "faustCompOperators", foreground: "a9b7c6"},
    {token: "identifier", foreground: "77CCFF"},
    {token: "string", foreground: "6a8759"},
    {token: "number", foreground: "6694b7"},
  ],
  colors: {
    'editor.foreground': '#AFB1B3',
 ...
 */
export const matchDocKey = (doc: TFaustDocs, model: editor.ITextModel, position: Position): TMatchedFaustDoc | null => {
  ...
  while (column$ - 2 >= 0 && line[column$ - 1] === ".") {
    ...
    column$ = (prefixWord?.startColumn || 0) - 1;
  }
...
export const getProviders = async (faust: FaustModule): Promise<FaustLanguageProviders> => {
  ...
  try {
    ...
    primDocs = await Faust2Doc.parse("primitives.lib", async (fileName: string) => Promise.resolve(primitives));
  } catch (e) {
    console.error(e);
      ...
      if (matched) {
        ...
        return {
          ...
          contents: [
            ...
            {value: prefix.length ? `[Detail...](${faustDocURL}/${docSections[prefix.slice(0, 2).toString() as keyof typeof docSections]}/#${prefix.join(".")}${(doc.name || '').replace(/[[\]|]/g, "").toLowerCase()})` : "[Detail...](https://faustdoc.grame.fr/manual/syntax/index.html#faust-syntax)"}
          ]
        };
      }
      return null;
